...
const siteURLnput = document.getElementById('siteUrl');
...
function addBookmark(event){
    ...
    const siteUrl = siteURLnput.value;
...
function isDuplicateBookmark(siteName,siteUrl){
    ...
    return bookmarks.some(
        (bookmark)=>bookmark.name.toLowerCase() === siteName.toLowerCase() && bookmark.url.toLowerCase() === siteUrl.toLowerCase()
    );
}
...
function removeBookmark(bookmark){
    ...
    bookmarks = bookmarks.filter(
        (b) => b.name.toLowerCase() !== bookmark.name.toLowerCase() ||
